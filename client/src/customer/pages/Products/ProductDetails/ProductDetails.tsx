...
const ProductDetails = () => {
    ...
    const [weight, setWeight] = useState(1);     // default 1 kg
    ...
    const isInCart = cart?.cartItems?.some(
        (cartItem: any) => cartItem.product?.id === Number(productId)
    );
            ...
            <div className='grid grid-cols-1 lg:grid-cols-2 gap-10'>
                ...
                <section className='flex flex-col lg:flex-row gap-5'>
                    <div className='w-full lg:w-[15%] flex flex-wrap lg:flex-col gap-3'>
                        {products.product?.images.map((item: any, index: any) => <img onClick={() => setSelectedImage(index)} className='lg:w-full w-[50px] cursor-pointer rounded-md' src={item} alt="" />)}
                    </div>
