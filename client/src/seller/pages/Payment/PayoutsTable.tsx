...
const PayoutsTable = () => {
  ...
  React.useEffect(() => {
    dispatch(fetchPayoutsBySeller(localStorage.getItem("seller_jwt") || ""));
  }, [dispatch]);
                    ...
                    {item.orderItems.map((orderItem: OrderItem) =>
                      <div key={orderItem.id} className='flex gap-5'>
                        ...
                        <div className='flex flex-col justify-between py-2'>
                          <h1>{t("title_label")}: {orderItem.product.title}</h1>
                          <h1>{t("price_label")}: Rs.{orderItem.product.sellingPrice}</h1>
